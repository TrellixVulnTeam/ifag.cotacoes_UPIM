...
Ex:
fab --list
fab -H faegpesebem setup_server:target="test",document_root="/var/www/testes"
fab -H faegpesebem deploy:target="test",document_root="/var/www/testes"
fab -H faegpesebem deploy:target="test",document_root="/var/www/testes",reinstalldb=True
fab -H faegpesebem disable:target="test",document_root="/var/www/testes"
...
env.base_name = 'ifag.org.br'  # Nome principal do projeto nas configurações
...
def setup_server(target='prod', document_root='/var/www/'):
    ...
    config_ssh_key()
    new_git_url = env.git_url.replace(env.git_host, env.normalized_name)
    run('git clone %s %s' % (new_git_url, env.deploy_dir))
...
def config_ssh_key():
    ...
    """
    put(
        os.path.join(env.conf_root, 'id_rsa'),
        '~/.ssh/%s-id_rsa' % env.base_name,
        mode='600'
    )
    put(
        os.path.join(env.conf_root, 'id_rsa.pub'),
        '~/.ssh/%s-id_rsa.pub' % env.base_name,
        mode='600'
    )
    ...
    contains = 'Host {normalized_name}'.format(**kwargs)
    if not files.contains('~/.ssh/config', contains, exact=True):
        content = '''
Host {normalized_name}
    Hostname       {git_host}
    IdentityFile   ~/.ssh/{base_name}-id_rsa
    IdentitiesOnly yes
            '''.format(**kwargs)
        run("echo '%s' >> %s" % (content, '~/.ssh/config'))


def get_settings_value(name):
    with cd(env.deploy_dir), prefix('source venv/bin/activate'):
        values = (name, env.settings_module)
        return run(
            'echo "from django.conf import settings; '
            'print(settings.%s)" | '
            'python manage.py shell --settings %s' % values)
...
def defaults(target="prod", document_root="/var/www/"):
    ...
    env.dbname = env.normalized_name
    env.dbuser = env.normalized_name
    ...
    with lcd(env.local_dir):
        ...
        env.git_host = urlparse(env.git_url).netloc.split('@')[1]
